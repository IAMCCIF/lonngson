...
# ==========================================
# 【核心修复】必须在 import cv2 之前执行，强制 OpenCV 屏蔽 GStreamer
# ==========================================
os.environ["OPENCV_VIDEOIO_PRIORITY_LIST"] = "V4L2"  # 强制 OpenCV 只用基础的 V4L2
os.environ["GST_PLUGIN_SCANNER"] = "0"               # 试图阻止 GStreamer 扫描插件
os.environ["OPENCV_LOG_LEVEL"] = "FATAL"             # 屏蔽底层 C++ 报错输出
...
def ask_deepseek(food_name, now_time, health_state):
    ...
    try:
        ...
        return "大模型请求失败: " + str(e)


class LongxinDietAnalyzerApp:
    def __init__(self, root):
        self.root = root
        self.root.title("智能饮食分析系统 - 实时视觉版")
        ...
        if not self.cap.isOpened():
            messagebox.showerror("摄像头错误", "无法打开摄像头，请检查设备连接状态！")
        else:
    ...
    def setup_styles(self):
        ...
        style.configure("TRadiobutton", font=self.font_normal, background=self.color_card, foreground=self.color_text)
    ...
    def create_layout(self):
        ...
        self.frame_left = ttk.LabelFrame(self.root, text=" 实时画面 (支持自适应缩放) ", style="Card.TLabelframe")
        ...
        lbl_title = tk.Label(frame_header, text="智能饮食分析", font=self.font_title, bg=self.color_bg, fg=self.color_primary)
        lbl_title.pack(side=tk.LEFT, anchor=tk.CENTER)
        
        btn_exit = ttk.Button(frame_header, text="❌ 关 闭", style="Exit.TButton", command=self.on_closing)
        btn_exit.pack(side=tk.RIGHT, ipady=3, ipadx=5)

        frame_health = ttk.LabelFrame(self.frame_right, text=" 1. 请选择身体状况 ", style="Card.TLabelframe", padding=10)
        ...
        frame_result = ttk.LabelFrame(self.frame_right, text=" 2. 分析报告 ", style="Card.TLabelframe", padding=10)
    ...
    def update_camera_feed(self):
        if self.cap.isOpened():
            ...
            if ret:
                ...
                img.thumbnail((self.camera_width, self.camera_height), Image.Resampling.LANCZOS)
    ...
    def set_button_state(self, state):
        def update():
            if state == tk.DISABLED:
                self.btn_analyze.config(state=tk.DISABLED, bg="#D1C7BD", text="分析中，请稍候...")
            else:
                self.btn_analyze.config(state=tk.NORMAL, bg=self.color_primary, text="拍摄并进行智能分析")
        self.root.after(0, update)

    def start_analysis_thread(self):
        if self.current_frame is None:
            messagebox.showwarning("警告", "未获取到摄像头画面，请检查设备！")
            return
        if self.is_analyzing: return
            
        self.is_analyzing = True
        self.set_button_state(tk.DISABLED)
        self.log_message("已截取当前画面，开始智能分析...", is_clear=True)
    ...
    def run_analysis(self, frame):
        try:
            ...
            if self.model is None:
                self.log_message("初次加载 YOLO 模型，需稍等几秒...")
                from ultralytics import YOLO
                self.model = YOLO(model_path)
            
            self.log_message("YOLO 视觉识别中...")
            ...
            if not obj_list:
                self.log_message("未能识别到已知食物，请调整角度重试。")
                return
            ...
            self.log_message("-" * 60)
            self.log_message(f"【视觉捕获成功】\n识别对象: {target_food}\n当前时间: {now_t}\n健康状况: {user_status}")
            self.log_message("-" * 60)
            
            self.log_message("\n正在连接 DeepSeek 获取专业膳食指南...")
